...
@pytest.mark.asyncio
async def test_cross_encoder_threshold_does_not_filter_rrf_shortlist(monkeypatch):
    """A cross-encoder threshold must not be applied to lower-scale RRF scores."""
    limit = 2
    bm25_hits = [_edge(f'bm25-{i}', f'bm25-fact-{i}') for i in range(4)]
    cosine_distractor = _edge('cosine-1', 'cosine distractor')
    cosine_answer = _edge('cosine-2', 'Alice works at Zep')
    ranked_passages: list[str] = []
    ...
    async def fake_similarity(*args, **kwargs):
        return [cosine_distractor, cosine_answer]

    class RecordingCrossEncoder:
        async def rank(self, query: str, passages: list[str]):
            ranked_passages.extend(passages)
            scored = [
                (passage, 0.9 if passage == cosine_answer.fact else 0.1) for passage in passages
            ]
            scored.sort(key=lambda item: item[1], reverse=True)
            return scored
    ...
    assert cosine_answer.fact in ranked_passages
    assert [edge.uuid for edge in edges] == [cosine_answer.uuid]
    assert scores == [0.9]
