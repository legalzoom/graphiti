...
class NeptuneHasEpisodeEdgeOperations(HasEpisodeEdgeOperations):
    async def save(
        ...
        if tx is not None:
            result = await tx.run(HAS_EPISODE_EDGE_SAVE, **params)
        else:
            result = await executor.execute_query(HAS_EPISODE_EDGE_SAVE, **params)
    ...
    async def save_bulk(
        ...
        # HAS_EPISODE edges are saved individually since no bulk query exists
